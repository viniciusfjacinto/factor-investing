...
def plot_returns_bars(returns, benchmark=None,
                      ...
                      resample="A", title="Retornos", match_volatility=False,
...
def plot_longest_drawdowns(returns, periods=10, lw=1.5,
                           fontname='Arial', grayscale=False,
    ...
    _plt.yscale("symlog" if log_scale else "linear")
    if ylabel:
        ax.set_ylabel("Retorno acumulado", fontname=fontname,
                      fontweight='bold', fontsize=12, color="black")
        ax.yaxis.set_label_coords(-.1, .5)
...
def plot_distribution(returns, figsize=(10, 6),
    ...
    port['Semanal'] = port['Daily'].resample(
        'W-MON').apply(apply_fnc)
    port['Semanal'].ffill(inplace=True)

    port['Mensal'] = port['Daily'].resample(
        'M').apply(apply_fnc)
    port['Mensal'].ffill(inplace=True)

    port['Trimestral'] = port['Daily'].resample(
        'Q').apply(apply_fnc)
    port['Trimestral'].ffill(inplace=True)

    port['Anual'] = port['Daily'].resample(
        'A').apply(apply_fnc)
    port['Anual'].ffill(inplace=True)
    ...
    fig.suptitle("Quantis dos retornos\n", y=.99,
    ...
    if ylabel:
        ax.set_ylabel('Retornos', fontname=fontname,
                      fontweight='bold', fontsize=12, color="black")
        ax.yaxis.set_label_coords(-.1, .5)
