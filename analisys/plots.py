...
from analisys import (
    stats as _stats, utils as _utils, core as _core
)
...
def snapshot(returns,rf, grayscale=False, figsize=(10, 8),
             title='Retorno da estratégia', fontname='Arial', lw=1.5,
    ...
    if subtitle:
        axes[0].set_title("\n%s - %s ;  Sharpe: %.2f                      " % (
            ...
            returns.index.date[-1:][0].strftime('%e %b \'%y'),
            _stats.sharpe(returns,rf)
        ), fontsize=12, color='gray')

    axes[0].set_ylabel('Retorno acumulado', fontname=fontname,
    ...
    axes[2].set_ylabel('Retorno diário', fontname=fontname,
                       fontweight='bold', fontsize=12)
...
def returns(returns, benchmark=None,
            ...
            resample=None, ylabel="Retorno acumulado",
            subtitle=True, savefig=None, show=True):
...
def daily_returns(returns,
                  ...
                  log_scale=False, ylabel="Retorno",
                  subtitle=True, savefig=None, show=True):

    fig = _core.plot_timeseries(returns, None, 'Retornos diários',
                                ylabel=ylabel,
...
def yearly_returns(returns, benchmark=None,
    ...
    if benchmark is not None:
        title += '  vs Benchmark'
        benchmark = benchmark.resample('A').apply(
...
def histogram(returns, resample='M', fontname='Arial',
    ...
    return _core.plot_histogram(returns,
                                ...
                                grayscale=grayscale,
                                fontname=fontname,
                                title="Distribuição dos retornos %s" % title,
                                figsize=figsize,
                                ylabel=ylabel,
                                subtitle=subtitle,
                                compounded=compounded,
...
def drawdowns_periods(returns, periods=10, lw=1.5, log_scale=False,
...
def rolling_beta(returns, benchmark,
                 window1=126, window1_label="6-Meses",
                 window2=252, window2_label="12-Meses",
...
def rolling_volatility(returns, benchmark=None,
                       ...
                       lw=1.5, fontname='Arial', grayscale=False,
                       figsize=(10, 5), ylabel="Volatilidade",
                       subtitle=True, savefig=None, show=True):
    ...
    if benchmark is not None:
        benchmark = _stats.rolling_volatility(
            benchmark, period, periods_per_year)

    fig = _core.plot_rolling_stats(returns, benchmark,
                                   hline=returns.mean(),
                                   hlw=1.5,
                                   ylabel=ylabel,
                                   title='Janelas rolantes Volatilidade (%s)' % period_label,
                                   fontname=fontname,
                                   grayscale=grayscale,
                                   lw=lw,
                                   figsize=figsize,
...
def rolling_sharpe(returns, benchmark=None, rf=None,
                   period=126, period_label="6-Meses",
                   periods_per_year=252,
                   lw=1.25, fontname='Arial', grayscale=False,
                   figsize=(10, 5), ylabel="Sharpe",
                   subtitle=True, savefig=None, show=True):

    returns = _stats.rolling_sharpe(
        returns, rf, period, periods_per_year)

    if benchmark is not None:
        benchmark = _stats.rolling_sharpe(
            benchmark, rf, period,periods_per_year)

    fig = _core.plot_rolling_stats(returns, benchmark,
                                   hline=returns.mean(),
                                   hlw=1.5,
                                   ylabel=ylabel,
                                   title='Janelas rolantes Sharpe (%s)' % period_label,
                                   fontname=fontname,
                                   grayscale=grayscale,
                                   lw=lw,
                                   figsize=figsize,
...
def rolling_sortino(returns, benchmark=None, rf=None,
                    period=126, period_label="6-Meses",
                    periods_per_year=252,
                    lw=1.25, fontname='Arial', grayscale=False,
                    figsize=(10, 5), ylabel="Sortino",
                    subtitle=True, savefig=None, show=True):
    ...
    fig = _core.plot_rolling_stats(returns, benchmark,
                                   ...
                                   hlw=1.5,
                                   ylabel=ylabel,
                                   title='Janelas rolantes Sortino (%s)' % period_label,
                                   fontname=fontname,
                                   grayscale=grayscale,
                                   lw=lw,
                                   figsize=figsize,
...
def monthly_heatmap(returns, annot_size=10, figsize=(10, 5),
    ...
    if ylabel:
        ax.set_ylabel('Anos', fontname=fontname,
                      fontweight='bold', fontsize=12)
        ax.yaxis.set_label_coords(-.1, .5)
